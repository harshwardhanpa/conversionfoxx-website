        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, index) => (
            ...
            >
              <Link
                to={service.link}
                className={`group block h-full p-8 glass-card glass-card-hover relative overflow-hidden ${
                  service.highlight ? 'border-brand-accent/30 bg-brand-accent/5 shadow-[0_0_20px_rgba(255,106,61,0.1)]' : ''
                }`}
              >
                {service.highlight && (
                  <div className="absolute top-0 right-0 bg-brand-accent text-brand-bg text-[10px] font-bold px-3 py-1 rounded-bl-lg uppercase tracking-widest">
                    Flagship
                  </div>
                )}
                
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-6 bg-brand-accent/10 text-brand-accent">
                  <service.icon className="w-6 h-6" />
                </div>

                <h3 className="text-xl font-display font-bold mb-4 group-hover:text-brand-accent transition-colors">
                  {service.title}
                </h3>
                ...
                <div className="mt-auto flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-brand-accent opacity-0 group-hover:opacity-100 transition-opacity">
